...
export function createArtKit() {
  ...
  function geometry(key) {
    ...
    if (key === "softbox") {
      ...
      result = new THREE.CylinderGeometry(0.5, 0.5, 1, 10);
    else if (key === "pot")
      result = new THREE.CylinderGeometry(0.5, 0.36, 1, 8);
    else if (key === "ring")
      result = new THREE.TorusGeometry(0.5, 0.055, 4, 20);
  ...
  const box = (root, color, position, scale) =>
    part(root, "softbox", color, position, scale);
  ...
  function building(model) {
    ...
    const w =
      Number.isFinite(model.width) && model.width > 0 ? model.width : 2.6;
    const d =
      Number.isFinite(model.depth) && model.depth > 0 ? model.depth : 2.4;
    ...
    if (kind === "square") {
      ...
      for (const side of [-1, 1]) {
        box(
          root,
          "timber",
          [side * w * 0.34, 0.31, d * 0.28],
          [0.72, 0.13, 0.3],
        );
        box(
          root,
          "stone",
          [side * w * 0.34, 0.15, d * 0.28],
          [0.48, 0.3, 0.18],
        );
      }
      return fitted();
    ...
    if (kind === "noticeboard") {
      ...
      for (const x of [-w * 0.24, w * 0.24])
        box(root, "timber", [x, 0.78, 0], [0.13, 1.5, 0.13]);
    ...
    for (const x of [-wallW / 2 + 0.06, wallW / 2 - 0.06])
      box(
        root,
        "timber",
        [x, height / 2 + 0.13, front + 0.025],
        [0.09, height, 0.08],
      );
    ...
    if (workshop) {
      ...
      for (let bay = 0; bay < 3; bay++) {
        ...
        const roof = part(
          ...
          "#345b5a",
          [x, roofY, 0],
          [bayWidth, 1.25, wallD + 0.36],
        );
        roof.userData.architecture = "sawtooth-bay";
        const glazing = box(
          root,
          "window",
          [x + bayWidth / 2 + 0.018, roofY + 0.35, 0],
          [0.035, 0.37, wallD * 0.83],
        );
      ...
      box(
        root,
        home ? style.roof : "roofDark",
        [0, roofY + pitch / 2 + 0.01, 0],
        [0.15, 0.13, wallD + 0.4],
      );
    }
    box(
    ...
    box(
      root,
      "timber",
      [doorX, workshop ? 0.81 : 0.66, front + 0.045],
      [workshop ? 1.05 : lodge ? 0.75 : 0.55, workshop ? 1.25 : 0.95, 0.11],
    );
    box(
      root,
      workshop ? "navy" : home ? style.door : "teal",
      [doorX, workshop ? 0.8 : 0.65, front + 0.112],
      [workshop ? 0.94 : lodge ? 0.63 : 0.43, workshop ? 1.12 : 0.82, 0.045],
    );
    part(
      root,
      "sphere",
      "brass",
      [doorX + 0.12, 0.64, front + 0.15],
      [0.045, 0.045, 0.045],
    );
    ...
    if (workshop) {
      ...
      box(
        root,
        "timber",
        [wallW * 0.21, 0.55, front + 0.3],
        [wallW * 0.42, 0.12, 0.36],
      );
      part(
        root,
        "cylinder",
        "brass",
        [wallW * 0.2, 0.7, front + 0.3],
        [0.18, 0.18, 0.18],
      );
      window(root, 0, 1.87, front + 0.07, wallW * 0.72, 0.32);
    } else {
      for (const x of [-wallW * 0.32, wallW * 0.32])
        window(root, x, 0.96, front + 0.07, wallW * 0.16, 0.44);
    }
    if (tall || lodge) {
    ...
    if (lodge) {
      ...
      box(
        ...
        "teal",
        [0, 1.46, front + 0.49],
        [wallW * 1.02, 0.14, 1.15],
      ).rotation.x = 0.12;
      for (const fraction of [-0.44, -0.18, 0.18, 0.44])
        box(
          root,
          "timber",
          [wallW * fraction, 0.77, front + 0.94],
          [0.09, 1.24, 0.09],
        );
      for (const side of [-1, 1])
        box(
          root,
          "cream",
          [side * wallW * 0.31, 0.59, front + 0.94],
          [wallW * 0.26, 0.1, 0.09],
        );
      box(
        root,
        "timber",
        [-wallW * 0.32, 0.38, front + 0.34],
        [wallW * 0.26, 0.13, 0.37],
      );
    } else {
      box(
        root,
        "stone",
        [-wallW * 0.28, roofY + 0.43, -wallD * 0.2],
        [0.29, 0.82, 0.31],
      );
      box(
        root,
        "cream",
        [-wallW * 0.28, roofY + 0.84, -wallD * 0.2],
        [0.38, 0.12, 0.39],
      );
    }
  ...
  function tree(seed = 0) {
    ...
    const n =
      typeof seed === "number"
        ? seed
        : [...String(seed)].reduce((sum, c) => sum + c.charCodeAt(0), 0);
