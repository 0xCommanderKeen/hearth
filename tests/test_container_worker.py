...
from hearth.process_mock import read_request, worker
from test_container_rehearsal import Docker
...
def system(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hearth.container_rehearsal.LocalDocker", lambda: fake_docker)
    ...
    monkeypatch.setattr("hearth.process_mock.subprocess.Popen", Spawn)
...
def test_cancellation_during_create_retains_unknown_claim_without_dispatch(system, monkeypatch):
    ...
    monkeypatch.setattr("hearth.container_rehearsal.LocalDocker", lambda: cancelled_create)
    folder = dispatch(system)
