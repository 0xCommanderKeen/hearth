...
- Fix Hamlet home occupancy and hidden noticeboard holds; bound portrait work per
  frame, stop idle village animation and avoid repeated room layout updates (#271).
...
- Add selected Discord channel setup, authenticated CLI configuration/revocation,
  ...
  keep their conversation busy even after the original closed transcript was pruned;
  preserve native call-ID uniqueness across management and communications receipts.
...
- Keep sandboxed management catalogs on the shared run volume, recognize Docker's
  ...
  and show resident activity with safe failure explanations.
