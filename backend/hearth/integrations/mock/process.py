...
from hearth.storage.artifacts import MAX_ARTIFACT, sync_directory
        ...
        try:
            ...
            if result.status in {"succeeded", "failed", "cancelled"}:
                ...
                if request["boundary"] == "container":
                    from hearth.integrations.mock.container_worker import validate_result
            ...
            with (folder / "worker.lock").open("a") as lock:
                ...
                if request["boundary"] == "container":
                    from hearth.integrations.mock.container_worker import reconcile
...
def run_child(folder: Path, request: dict) -> Evidence:
    child = subprocess.Popen(
        [sys.executable, "-I", "-m", "hearth.integrations.mock.process", "child", str(folder)],
        cwd=folder,
...
def worker(folder: Path) -> None:
    with (folder / "worker.lock").open("a") as lock:
        ...
        if request["boundary"] == "container" and not (folder / "cancel.json").exists():
            from hearth.integrations.mock.container_worker import run_container
