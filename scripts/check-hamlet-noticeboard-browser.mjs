...
const output = new URL(
  "../docs/evidence/hamlet-noticeboard-2026-09-13/",
  import.meta.url,
);
...
try {
  ...
  await page.getByRole("button", { name: "Noticeboard demo" }).click();
  await page
    .getByRole("region", { name: "Village noticeboard" })
    .scrollIntoViewIfNeeded();
  assert.equal(
    await page
      .getByRole("link", { name: "Mara · Run completed" })
      .getAttribute("href"),
    "#runs/demo-finished-run",
  );
  assert.equal(
    await page.getByRole("link", { name: /Iris · Outcome unknown/ }).count(),
    1,
  );
  await snap("noticeboard-desktop");
  ...
  await snap("noticeboard-mobile");
  await page.setViewportSize({ width: 1440, height: 1100 });
  checks.push(
    "Noticeboard: completed run link, unknown outcome, retained letter and 320px layout",
  );
  ...
  for (const [name, kind] of [
    ...
    poses.push(
      await page.evaluate(() =>
        window.roomScene.children
          .find((c) => c.userData.agentId)
          .userData.arms.map((a) => a.rotation.toArray()),
      ),
    );
    assert.equal(
      await page.evaluate(
        (kind) => !!window.roomScene.getObjectByName(`work-prop:${kind}`),
        kind,
      ),
      true,
    );
  ...
  checks.push(
    "Four distinct static working poses and held props; reduced-motion and offline pauses retained",
  );
