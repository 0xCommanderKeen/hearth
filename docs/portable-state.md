...
It does not import or activate residents. The format is deliberately pinned to
...
Still required: an isolated read-only importer, repeated-import behavior, a detailed
semantic diff, reverse export, explicit compatibility mappings, memory/capability
models, ownership contention and rollback rehearsals. Cross-version conversion and
live migration acceptance are not established by this export. No HTTP export or
