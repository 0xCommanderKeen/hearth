...
is queued in that transaction when needed. This removes Warren's need to reconcile
an operational database with separately authoritative event history, at the cost of
one host's availability and serialized writes. The database owns current state;
