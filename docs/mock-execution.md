...
The executor sends canonical JSON context version 6: run/task/resident identities,
the declaration revision pinned at admission, its purpose and skill text, the task instruction and
the fixed synthetic notes and pinned resident memory. The scoped runtime context route uses the same reader.
